...
def transform(image, target, split):
    ...
    new_boxes = target["boxes"].type(torch.float64)
    new_labels = torch.unsqueeze(target["labels"].type(torch.float64), 1)
    new_boxes = torch.cat((new_boxes, new_labels), 1).numpy()

    # Skip the following operations for evaluation/testing
    seq = Sequence(
        [RandomHSV(40, 40, 30), RandomTranslate(), RandomRotate(10), RandomShear()]
    )
    if split == "TRAIN":
        # A series of photometric distortions in random order, each with 50% chance of occurrence, as in Caffe repo
        new_image = photometric_distort(new_image)  # input: PIL image
        new_image = cv2.cvtColor(np.asarray(new_image), cv2.COLOR_RGB2BGR)[:, :, ::-1]
        new_image, new_boxes = seq(new_image.copy(), new_boxes.copy())

    new_image = FT.to_tensor(new_image)

    target["boxes"] = torch.FloatTensor(new_boxes[:, :4])
    target["labels"] = torch.from_numpy(new_boxes[:, -1]).type(torch.int64)
