...
def calculate_mAP(det_results, ground_truths):
    """
    ...
    (deprecated):param true_difficulties: list of tensors, one tensor for each image containing actual objects' difficulty (0 or 1)
    :return: list of average precisions for all classes, mean average precision (mAP)
    """
    det_boxes, det_labels, det_scores = [], [], []
    true_boxes, true_labels = [], []
    for det_dict, gt_dict in zip(det_results, ground_truths):
        det_boxes += det_dict["boxes"]
        det_labels += det_dict["labels"]
        det_scores += det_dict["scores"]
        true_boxes += gt_dict["boxes"]
        true_labels += gt_dict["labels"]
    assert (
        len(det_boxes)
        == len(det_labels)
        == len(det_scores)
